...
/*
    ...
    1. 캐시 구조체
        ...
        -> 노드는 key : GET http://local../home.html HTTP/1.1), value(응답
   데이터), 링크드 리스트를 위한 포인터 (next, prev)가 필요하다.

    3. 데이터 찾기
        1. key를 인자로
        2. 데이터를 찾으면 1 반환 (buf 값을 인자로 받고 넣어주면 데이터는 거기에
   값 채우기)
        3. 데이터를 못 찾으면 0 반환
...
int insert_cache(cache *target_cache, char *key,
                 char *value);  // 캐시에 값 넣기
