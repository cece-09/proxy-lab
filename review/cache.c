...
cache_node *new_cache_node(
    char *key, char *value) {  // key는 uri가 됨, value는 서버의 응답 데이터
  // 캐시 노드 구조체 포인터 생성 및 할당
  cache_node *node = malloc(sizeof(cache_node));
  // 키(key) 메모리 할당 및 복사
  node->key = malloc(
      strlen(key) +
      1);  // 키 문자열의 길이 + 1 크기의 메모리 할당 ,+1하는 이유 널문자 포함
  strcpy(node->key, key);  // 키 문자열을 복사하여 저장
  // 값(value) 메모리 할당 및 복사
  node->value =
      malloc(strlen(value) + 1);  // 값 문자열의 길이 + 1 크기의 메모리 할당
  strcpy(node->value, value);  // 값 문자열을 복사하여 저장
  // 초기화된 캐시 노드 구조체 포인터 반환
  return node;
}  // 이렇게 초기화된 캐시 노드는 캐시에 추가될 때 사용되며, 키와 값이 복사되어
   // 보관
...
//  3. 데이터 찾기 (find_value)
//         1. key를 입력하면 데이터 찾기
//         2. 데이터를 찾으면 1 반환 (buf 값을 인자로 받고 넣어주면 데이터는
//         버퍼에 값 채우기)
//             이후 찾은 데이터를 삭제하고, 링크드 리스트 가장 앞에
//             넣어주자(LRU를 위해)
//         3. 데이터를 못 찾으면 0 반환
int find_cache(cache *target_cache, char *key, char *buf) {
  ...
  while (start != NULL) {
    if (strcmp(start->key, key) == 0) {  // 같은 키 값을 찾으면?
      ...
      insert_cache(target_cache, start->key,
                   start->value);  // 찾은 값을 연결 리스트 가장 앞에 넣는다.
...
int insert_cache(cache *target_cache, char *key, char *value) {
  ...
  while (target_cache->size + strlen(value) >
         MAX_CACHE_SIZE) {  // 넣을 수 있는 사이즈가 될 때까지 반복
  ...
  // 2. 노드를 캐시에 추가한다.
  // 2.1. 새로운 노드의 next 포인터를 root 노드로 한다.
  new_node->next = target_cache->root;
  // 2.2. root 노드에 prev 포인터가 새로운 노드를 가리키도록 한다.
  // 만약 root가 널이면?
