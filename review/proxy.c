...
int main(int argc, char **argv) {
  ...
  while (1) {
    ...
    *connfd = Accept(listenfd, (SA *)&clientaddr, &clientlen);
    ...
    Pthread_create(
        &tid, NULL, thread,
        connfd);  // 새로운 피어 쓰레드 생성 -> thread 함수를 돌게 함.
    // 위 함수가 리턴될 때, 메인 쓰레드와 새로 생성된 피어 쓰레드는 동시에 돌고
    // 있으며, tid는 새로운 쓰레드의 ID를 갖고있다.
  }
...
void *thread(void *vargp) {
  ...
  // pthread_self 로 리턴받은 자신의 ID가 담긴 tid를 pthread_detach 에
  // 넣으므로써 자신을 분리함.

  // 이렇게 분리 하는 이유는 기본적으로 쓰레드는 연결 가능 상태로 생성되며,
  // 연결 가능 상태인 쓰레드는 다른 쓰레드에 의해 청소되기 전까지 자신의 메모리
  // 자원들이 남아있기 때문에 pthread_exit로 삭제시키거나 이나 pthread_detach 로
  // 분리해야 한다. 분리된 쓰레드는 쓰레드가 종료할 때 자신의 자원들 또한
  // 반환된다.
  Free(vargp);
  doit(connfd);
  Close(connfd);
...
void doit(int fd) {
  ...
  if (find_cache(proxy_cache, uri,
                 data)) {  // findcache는 찾으면 1, 못찾으면 0 리턴함 \ 찾은
                           // 캐시 데이터를 data에 저장
    ...
    while ((n = Rio_readlineb(&server_rio, buf, MAXLINE)) != 0) {
      ...
      // 응답 데이터의 크기가 최대 캐시 객체 크기를 넘지 않으면 데이터를 임시
      // 저장
    ...
    if (cache_size_check == 1) insert_cache(proxy_cache, uri, data);
  }
}

void make_header(char *header, char *hostname, char *path, rio_t *rio,
                 char *method) {
  char buf[MAXLINE], r_host_header[MAXLINE], request_header[MAXLINE],
  ...
  sprintf(header, "%s%s%s%s%s%s%s", request_header, r_host_header,
          connection_header, proxy_connection_header, user_agent_hdr,
          other_header, "\r\n");
  return;
}
